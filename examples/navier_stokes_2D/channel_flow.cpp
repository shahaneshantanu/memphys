...
int main(int argc, char *argv[])
{
    MPI_Init(&argc, &argv);
    clock_t t0 = clock();
    PARAMETERS parameters("parameters_file.csv", "/home/shantanu/Desktop/All Simulation Results/Meshless_Methods/CAD_mesh_files/Rectangle/Rectangle_n_10.msh");
    ...
    double x, y;
    double Re = 40.0, ly = 1.0, lx = 10.0, u_bulk = 1.0;
    ...
    { // Set boundary conditions here
        ...
        if (points.boundary_flag[iv])
        {
            ...
            if (fabs(x - lx) < 1E-5) //outlet
                u_dirichlet_flag[iv] = false, v_dirichlet_flag[iv] = false, p_dirichlet_flag[iv] = false;
    ...
    for (int it = 0; it < parameters.nt; it++)
    {
        ...
        if (tecplot_it_frequency > 0 && it > 0)
            if (it % tecplot_it_frequency == 0 || total_steady_err < parameters.steady_tolerance || it == parameters.nt - 1)
                write_navier_stokes_tecplot_temporal_fields(points, parameters, u_new, v_new, u_new, p_new, it);
