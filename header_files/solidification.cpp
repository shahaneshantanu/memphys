...
void SOLIDIFICATION::write_tecplot_temporal_header(POINTS &points, PARAMETERS &parameters)
{
    string output_file = parameters.output_file_prefix + "_tecplot_temporal.plt";
    FILE *file;
    file = fopen(output_file.c_str(), "w");
    if (parameters.dimension == 2)
        fprintf(file, "VARIABLES=\"x\", \"y\", \"T\", \"fs\"\n");
    else
        fprintf(file, "VARIABLES=\"x\", \"y\", \"z\", \"T\", \"fs\"\n");
    fclose(file);
}

void SOLIDIFICATION::write_tecplot_temporal_fields(POINTS &points, PARAMETERS &parameters, Eigen::VectorXd &T_new, Eigen::VectorXd &fs_new, int it)
{
    ...
    if (dim == 3)
    {
        ...
        for (int iv0 = 0; iv0 < points.nv_original; iv0++)
        {
            ...
            fprintf(file, " %.16g %.16g ", T_new[iv1], fs_new[iv1]);
            fprintf(file, "\n");
        }
    }
    else
